...
后续决策把上文“`wait_agent` 的公开投影只包含两类相互独立的事实”修订为**三类**：

- 会话事件：`reply`、`final_report`、`idle` 和终止/故障的 `terminal`；事件说明这次等待为何被唤醒，仍只由 Pi 已接纳的父端消息和真实生命周期事实登记，保持不可覆盖、至多一次。
- 生命周期快照：独立返回七个生命周期状态之一及其安全修订/故障字段；事件名称不能覆盖或推断 `state`。
- **等待层释放事实**（本次新增的第三类）：`timeout`、`batch_released` 和 `woken`。它们只说明“这次等待调用为什么结束”，不指向任何目标的会话事实，不携带 `state`/`revision`，不进入会话事件闭集，也不参与事件不可覆盖、至多一次和回合水位机制。

第三类由 [父代理入站消息对阻塞中 wait_agent 的唤醒](10-wait-wake-on-parent-input.md) 引入：`woken`（`wake_reason: "parent_input"`）用于在子代理阻塞于 `wait_agent` 时由父代理入站消息释放本次等待，与 `timeout` 同构。上文其余结论（不建模任务与任务结果、事件与状态独立投影、通知水位、报告不重复携带正文）不变。
